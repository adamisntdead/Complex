...
module.exports.abs = function (x) {
  return Math.sqrt(Math.pow(x.Re, 2) + Math.pow(x.Im, 2));
};

module.exports.conj = function (x) {
...
module.exports.sqrt = function (x) {
  ...
  var im = sgn(x.Im) * Math.sqrt((0 - x.Re + Math.sqrt(Math.pow(x.Re, 2) + Math.pow(x.Im, 2))) / 2);
...
module.exports.toString = function (x) {
  if (x.Re === 0) {
    return x.Im + 'i';
  }
  if (x.Im >= 0) {
